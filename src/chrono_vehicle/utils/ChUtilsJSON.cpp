...
std::shared_ptr<ChPowertrain> ReadPowertrainJSON(const std::string& filename) {
    ...
    if (subtype.compare("SimplePowertrain") == 0) {
        powertrain = std::make_shared<SimplePowertrain>(d);
    } else if (subtype.compare("SimpleCVTPowertrain") == 0) {
        powertrain = std::make_shared<SimpleCVTPowertrain>(d);
    } else if (subtype.compare("SimpleMapPowertrain") == 0) {
        powertrain = std::make_shared<SimpleMapPowertrain>(d);
    } else if (subtype.compare("ShaftsPowertrain") == 0) {
        powertrain = std::make_shared<ShaftsPowertrain>(d);
    } else {
        throw ChException("Powertrain type not supported in ReadPowertrainJSON.");
    }
    return powertrain;
}
